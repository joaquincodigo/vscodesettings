...
def visualize_first_100_puzzles(csv_file, window_size=600, delay=1.0):
    """Visualize the first 100 puzzles from the CSV file."""
    try:
        visualizer = ChessPuzzleVisualizer(window_size)
        ...
        with open(csv_file, 'r') as f:
            ...
            for i, row in enumerate(reader):
                ...
                # Allow exiting the visualization early
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        visualizer.close()
                        return
