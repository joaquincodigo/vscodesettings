...
class ChessPuzzleVisualizer:
    ...
    def animate_move(self, board, move):
        """Animate a chess piece moving from one square to another with precise capture timing."""
        ...
        for frame in range(self.animation_speed + 10):
            ...
            # Draw the board state and the moving piece
            self.draw_board()
            
            # Draw the current pieces on the board, including the captured piece until the last frame
            if frame < self.animation_speed + 10 - 1:
                self.draw_position(board)
            else:
                # On the last frame, update temp_board to remove the captured piece
        ...
        # Finally, push the move to update the actual board
        board.push(move)


    def visualize_puzzle(self, fen, moves_str, delay=0.3):
