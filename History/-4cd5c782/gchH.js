window.addEventListener("load", () => {
  ...
  function main() {
    ...
    const intervalId = setInterval(() => {
      debug_frameCounter++;
      if (debug_frameCounter > 14) { // prevent infinite running and memory leak
...
// The snake is drawn on the grid.

// Frame 2 (First renderFrame() Call)
// renderFrame() removes the entire current snake body from the grid.
// moveSnake() is called, which removes the tail and adds a new head.
// The new snake is rendered, but since the entire body was initially erased, the first chunk (previous head) appears missing.
// Frame 3 (Second renderFrame() Call)

// The issue repeats one more time.
// From frame 4 onward, it stabilizes.